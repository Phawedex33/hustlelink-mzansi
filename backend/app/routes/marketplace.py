"""
Marketplace routes: service listings and booking management.
"""
...
def create_service():
    ...
    if not user or not user.provider_profile:
        return error_response("User is not a provider", 403, "forbidden")

    if not user.is_active:
        return error_response("User account is suspended", 403, "forbidden")

    if not user.provider_profile.can_list_services:
        return error_response(
            "Provider profile must be complete and approved by admin "
...
def create_booking():
    ...
    if not user or not user.is_active:
        return error_response(
            "Unauthorized or account suspended",
            403,
            "forbidden",
        )
    ...
    if not service_id or not scheduled_at_str:
        return error_response(
            "Service ID and schedule time required", 400, "bad_request"
        )
    ...
    try:
        ...
        return error_response(
            "Invalid date format. Use ISO format.", 400, "bad_request"
        )
...
def list_user_bookings():
    ...
    if user.provider_profile:
        provider_bookings = (
            Booking.query.join(Service)
            .filter(Service.provider_id == user.provider_profile.id)
            .all()
        )
